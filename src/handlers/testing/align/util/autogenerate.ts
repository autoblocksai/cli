...
export async function writeEvaluatorTemplates(args: {
  ...
  await ensureEvaluatorsDirectoryExists({
    ...
    testSuiteDirectory: args.testSuiteDirectory,
  });
...
async function ensureEvaluatorsDirectoryExists(args: {
  ...
  const evaluatorsDirectory = `${args.testSuiteDirectory}/${EVALUATORS_DIRECTORY}`;
  try {
...
async function writeTemplate(args: {
  ...
  const filepath = `${args.testSuiteDirectory}/${EVALUATORS_DIRECTORY}/${filename}`;
