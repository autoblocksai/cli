...
import { AutogeneratedEvaluator, Language } from './models';
...
}): Promise<AutogeneratedEvaluator[]> {
  await ensureEvaluatorsDirectoryExists({
    language: args.language,
    runTestSuiteCalledFromFilepath: args.runTestSuiteCalledFromFilepath,
  });
...
async function ensureEvaluatorsDirectoryExists(args: {
  ...
  const evaluatorsDirectory = `${args.runTestSuiteCalledFromFilepath}/${EVALUATORS_DIRECTORY}`;
  try {
    await fs.access(evaluatorsDirectory);
  } catch {
    await fs.mkdir(evaluatorsDirectory);
  }
...
async function writeTemplate(args: {
  ...
  const filepath = `${args.runTestSuiteCalledFromFilepath}/${EVALUATORS_DIRECTORY}/${filename}`;
