...
interface AppProps {
  ...
  requestNextTestCaseResult: AsyncHandlerOf<string>;
  onTestCaseResultGraded: AsyncHandlerOf<GradedTestCaseResult>;
  autogenerateEvaluators: () => Promise<AutogeneratedEvaluator[]>;
  onExit: Handler;
}
...
function App(props: AppProps) {
  ...
  const [autogeneratedEvaluators, setAutogeneratedEvaluators] = useState<{
    isLoading: boolean;
    data: AutogeneratedEvaluator[] | undefined;
  }>({
  ...
  const requestNextTestCaseResult = async (args: {
    ...
    const testCaseHashes = props.testCaseHashes();
    const firstHash = testCaseHashes[0];
    ...
    if (args.prevHash) {
      ...
      await props.requestNextTestCaseResult(nextTestCaseHash);
    } else {
      // Request the first test case if no previous hash is provided
      await props.requestNextTestCaseResult(firstHash);
    }
  };
